...
def main(): 
    ...
    pos = torch.tensor(np.linspace(0, 1, s), dtype=torch.float)
    pos = pos.reshape(-1, 1).unsqueeze(0)
    pos_train = pos.repeat(ntrain, 1, 1)
    pos_test = pos.repeat(ntest, 1, 1)    

    train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(pos_train, train_xy, train_s), batch_size=args.batch_size,
                                                shuffle=True)
    test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(pos_test, test_xy, test_s), batch_size=args.batch_size,
                                                shuffle=False)
