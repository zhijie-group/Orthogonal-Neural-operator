...
def main(): 
    ...
    gridx = gridx.reshape(s1, 1, 1).repeat([ 1, s2, 1])
    gridy = torch.tensor(np.linspace(0, 1, s2), dtype=torch.float)
    gridy = gridy.reshape(1, s2, 1).repeat([ s1, 1, 1])
    pos = torch.cat((gridx, gridy), dim=-1)
    pos = pos.reshape(-1, 2).unsqueeze(0)
    ...
    if args.model in ['ONO', 'ONO2']:
        if args.model == 'ONO2':
            model = ONO2(n_hidden=args.n_hidden, n_layers=args.n_layers, space_dim=2, fun_dim=2, n_head = args.n_heads, momentum=args.momentum, orth=args.orth, psi_dim=args.psi_dim, mlp_ratio=args.mlp_ratio, attn_type=args.attn_type).cuda()
        else:
